...
#def integrand_at_rtp(r, theta, phi, bra_func, ket_func, g_alpha, g_norm, Z, rc, static_args=(0, 1)):
def integrand_at_rtp(r, theta, phi, bra_func, ket_func, g_alpha, g_norm, Z, rc):
    ...
    bra = jax.vmap(bra_func, in_axes=(0, None, None, None, None))(e_pos, g_alpha, g_norm, Z, rc)
    ket = jax.vmap(ket_func, in_axes=(0, None, None, None, None))(e_pos, g_alpha, g_norm, Z, rc)
...
#@jax.jit
@partial(jax.jit, static_argnames=['bra_func', 'ket_func', 'Z', 'rc'])
def integrand_at_rtp_H(r, theta, phi, bra_func, ket_func, g_alpha, g_norm, Z, rc):
    ...
    hess_ket = jax.vmap(lambda x: jnp.trace(jax.hessian(ket_func)(x, g_alpha, g_norm, Z, rc)))(e_pos)
...
@partial(jax.jit, static_argnames=['bra_func', 'ket_func', 'Z', 'rc'])
def test_integrand_at_rtp(bra_func, ket_func, g_alpha, g_norm, Z, rc, n_points=16):
    ...
    r_grid, theta_grid, phi_grid = jnp.meshgrid(rgrid, theta, phi, indexing='ij')
    w_r_grid, w_theta_grid, w_phi_grid = jnp.meshgrid(w_r, w_theta, w_phi, indexing='ij')
    ...
    S_vals = integrand_at_rtp(r_grid, theta_grid, phi_grid, bra_func, ket_func, g_alpha, g_norm, Z, rc)
    #print ('S_vals', S_vals.shape)
    H_vals = integrand_at_rtp_H(r_grid, theta_grid, phi_grid, bra_func, ket_func, g_alpha, g_norm, Z, rc)
...
def cusp_coeff_vec(g_alpha, g_norm, Z, rc):
    ...
    for i, bra_func in enumerate(basis_funcs):
        for j, ket_func in enumerate(basis_funcs):
            S_rc, H_rc = test_integrand_at_rtp(bra_func, ket_func, g_alpha, g_norm, Z, rc)
