...
ground state and every excited state, in the \emph{same} natural-orbital
basis.

This unlocks several quantum-chemistry workflows that have no
equivalent at the NN-VMC level. Transition density matrices
$\gamma^{(0k)}_{pq} = \langle \Psi_{\mathrm{NN}}^{(0)} | a_p^{\dagger}
a_q | \Psi_{\mathrm{NN}}^{(k)} \rangle$ follow immediately from the
recovered $\widehat{c}^{(0)}$ and $\widehat{c}^{(k)}$ by standard
CI-vector contractions, giving oscillator strengths and excited-state
