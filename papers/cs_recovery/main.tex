...
\paragraph{Production-grade FermiNet + Jastrow + backflow.}
We re-ran the aug-cc-pVDZ K=3 demonstration with the production
FermiNet ansatz (\texttt{ferminet\_jastrow.yaml}): embedding dim
256, 4 GNN interactions, 16 determinants, native backflow,
deep-Jastrow head, with $738{,}560$ parameters per state. Trained
for 3000 SR iters on 256 walkers (vs.\ 2000/64 for PsiFormer-small),
the GS reached $E = -76.38\,E_{\rm h}$ (vs.\ PsiFormer-small's
$-76.27$), capturing 110 m$E_{\rm h}$ more correlation. The K=3
trace converged to $\mathrm{Tr}(E_\Psi) = -226.0\,E_{\rm h}$ vs.\
PsiFormer's $-218.6\,E_{\rm h}$. The match-overlap of the recovered
excited state with the CAS(8,8) 1B$_1$ root improved from 0.34
to 0.48, the NTO weights stayed at $99.4\%$ on eigenstate 2 and
$97.2\%$ on the selected excited state 3 (vs.\ 98.3\% for
PsiFormer), and the oscillator strength rose from 0.027 to 0.035
(closer to the experimental $\sim 0.04$).

The CAS+PT2 vertical excitation, however, remained at 9.97\,eV.
This is not an artifact of the FermiNet training; it is the
direct signature of the bottleneck having shifted from NN quality
to CAS(8,8) truncation of the NEVPT2 reference: the recovered
c-vectors of both ansatzes project onto the same lowest CAS(8,8)
eigenvector via the maximum-overlap matching, and NEVPT2's PT2
correction is computed from that CAS root, not from the NN-recovered
c-vector directly. With FermiNet+J, the recovered c-vectors live
$\sim 20$ orders of magnitude further out of the basis
(${\rm proj\_mass}^{(1)} = 5\times 10^{-20}$ vs.\ $9.7\times 10^{-3}$
for PsiFormer-small), reflecting the basis-extension correlation
the larger network captures --- which is precisely the correlation
that aug-cc-pVDZ + CAS(8,8) cannot represent. The CS pipeline as
formulated extracts only the basis-resolved component, so the
NEVPT2 result is unchanged. The methodology improvement
demonstrated is on the orbital decomposition (NTO purity) and on
state-quality diagnostics (match-overlap, $f$), not on the absolute
energy.

To unlock the FermiNet+J correlation through NEVPT2 we would need
either a larger active space (CAS(8,12) including the 3s$_{a_1}$
Rydberg orbital explicitly) or a direct $\langle\Psi_{\rm NN}|H|\Psi_{\rm NN}\rangle$
energy evaluation rather than CS projection followed by basis-CASCI.

\paragraph{CAS(12,(4,4)) closes the active-space bottleneck.}
We re-ran the FermiNet+Jastrow K=3 spectroscopy with an extended
CAS(12,(4,4)) reference --- the 8 valence orbitals plus 4 additional
low-lying virtuals, which by construction include the 3s$_{a_1}$
Rydberg orbital of the $\tilde A^1 B_1$ state. The CAS active-space
FCI grows from 4900 to $\sim 245{,}000$ determinants (CAS(8,(4,4)) vs
CAS(12,(4,4)) string counts; both are easily tractable in PySCF).
Re-running the spectroscopy step on the existing FermiNet+Jastrow
checkpoints (no retraining --- only the CASCI reference and the
active-space contracts change) gave:

\begin{itemize}
\item $\Delta E$ drops $9.97 \to 9.63\,{\rm eV}$ ($-0.34$\,eV
  improvement, now $0.7$\,eV below CIS/cc-pVDZ),
\item Oscillator strength rises $0.035 \to 0.102$ (near experimental
  scale $\sim 0.04$),
\item NTO decomposition becomes multi-pair: $69\%, 24\%, 6\%$
  (vs.\ $97\%$ single-pair under CAS(8,8)), which is the
  \emph{physically expected} structure of a Rydberg state involving
  several diffuse virtual orbitals,
\item NEVPT2 PT2 shrinks from $-235$ to $-201$ m$E_{\rm h}$ on
  the excited (more correlation is now in the reference,
  less in the perturbation), confirming the active-space pickup.
\end{itemize}

\noindent The 97\,\% single-pair NTO of the CAS(8,8) result was an
\emph{artifact of the truncated active space} forcing the recovered
$\widehat c$ projection onto a single orbital pair; the true Rydberg
state has dominant 3s$_{a_1}$ character mixed with $3p$ and $3d$
contributions, which only emerge when the active space contains
those orbitals explicitly. This is a useful methodological lesson:
``clean'' single-NTO transitions in the small-CAS regime can mask
genuine multi-orbital physics, and the choice of active space
materially affects the recovered chemistry, not just the energy.

The remaining $\sim 2.2$\,eV gap to experimental 7.4\,eV is now
dominated by the basis-set incompleteness of aug-cc-pVDZ ($\sim
1.5$\,eV at the EOM-CCSD CBS limit per
ref.~\cite{schreiber2008benchmarks}) and the residual K=3 span
imperfection ($\sim 0.5$\,eV); the active-space contribution has
...
This is a real-world bottleneck for paper-grade applications:
basis-set extension alone does not improve the result without
proportional Pfau-NES training-budget extension. We submitted a
followup at 8000 iters / 512 walkers; if this recovers the basis
benefit then the limiting factor in our pipeline is genuinely
``Pfau-NES sampling vs.\ basis size'', a quantifiable observation
the paper documents but does not exhaustively solve.
