...
On a single GH200 we attempted $K=4$ but hit a jaxlib/cuDNN
``\texttt{double free in tcache 2}'' during JIT compilation
triggered by the $K^2 = 16$ static-unrolled matrix-construction loop
in our current implementation; a re-implementation using
\texttt{jax.vmap} over the configuration axis avoids the unrolling
and should remove this limitation, which we defer to follow-up.

\bigskip
Table~\ref{tab:h2o_ccpvdz_comparison} now shows the K=3 row as the
canonical paper-grade Pfau-NES + CS + NEVPT2 entry; the K=2 row
remains for the methodology contrast.
