...
\end{itemize}
A clean solution likely requires either (a) an absolute-overlap
penalty $|\sum_I \widehat{c}^{(0)}_I c^{(1)}_{{\rm raw},I}|^{2}$
without the degenerate scale invariance, paired with a strong
energy regulariser to constrain $\Psi_1$'s scale; (b) a
constrained-optimisation approach that projects the gradient onto
the orthogonality manifold rather than minimising a soft penalty;
or (c) an architectural change so that $\Psi_{\rm NN}$ lies in (or
is biased toward) the chosen Gaussian basis, eliminating the
basis-incompleteness pathway entirely. These are interesting
directions for follow-up work; the \texttt{OmegaQMC.vmcopt\_nn\_nes}
module exposes three skeleton implementations along the relevant
axes.
