...
bank per state --- would yield CI vectors $\{\widehat{c}^{(k)}\}$ for
the ground state and every excited state, in the \emph{same}
natural-orbital basis. Implementing the penalty-method NES-VMC
optimisation inside the current \textsc{OmegaQMC} framework
constitutes a substantial extension to the existing
single-state SR driver and is left to a follow-up
work; the recovery pipeline itself requires no modification, since
$f_I$ has no explicit dependence on which state $\Psi_{\rm NN}^{(k)}$
represents.
