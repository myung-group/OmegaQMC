...
orbital frame (single-reference); and selected-CI warm-start. On
converged \ce{BeH2}/cc-pVDZ, NEVPT2 driven from $\widehat c$
obtains energies $0.4$--$4.0$\,m$E_{\rm h}$ \emph{below}
CASSCF+NEVPT2 at matched CAS sizes, with no orbital optimisation
step; the NN-NO orbital frame additionally lowers the CCSD and
CCSD(T) total energies vs the standard HF reference. (iii)~\emph{Excited-state spectroscopy from NES-VMC
...
the framework already produces from $\widehat c$ (\S\ref{sec:rdm_apps}),
...
\noindent The MC-PDFT row will be filled with the result of
SLURM job 5439 (running). The pattern we expect: MC-PDFT at the
$\widehat c$-derived RDMs gives an independent estimate of the
basis-set-FCI energy that uses entirely different theoretical
machinery from NEVPT2 (DFT pair-density functional vs Dyall-$H_0$
perturbation theory). Agreement of the two estimates strengthens
the framework's claim that $\widehat c$ is a stable input to
multiple post-HF method families.
...
(\S\ref{sec:nn_no_self_consistent}); using these NN-NOs as the
...
method                                  & $E_{\rm tot}$ ($E_{\rm h}$) & gap to FCI (m$E_{\rm h}$) \\
\midrule
CCSD  (HF orbitals)                     & TBD & TBD \\
CCSD  (NN-NO orbitals from $\widehat c$) & TBD & TBD \\
CCSD(T) (HF orbitals)                   & TBD & TBD \\
CCSD(T) (NN-NO orbitals from $\widehat c$) & TBD & TBD \\
\midrule
FCI(cc-pVDZ)                            & $-15.8364$ & 0 \\
\bottomrule
\end{tabular}
...
\noindent SLURM job 5439 (running). The expected pattern: NN-NOs
import the multireference correlation captured by the
FermiNet+Jastrow trial into the orbital frame consumed by CC, so
CCSD on NN-NOs delivers a lower (i.e.\ better) energy than CCSD on
HF orbitals. This is the single-reference analog of
\S\ref{sec:caspt2_replacement}'s CASSCF-free NEVPT2 claim: NN-NOs
replace the orbital-optimisation step that HF would have left
suboptimal for correlation, and any post-HF method downstream
inherits that improvement.
