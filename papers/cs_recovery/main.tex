...
A pilot demonstration on \ce{BeH2} at $R_{\rm Be-H} = 1.33$~\AA\ in
cc-pVDZ illustrates the basic outcome. A FermiNet ansatz with a deep
Jastrow scalar correction was trained for 1000 SR iterations to
$E_{\rm NN} = -15.838(7)\,E_{\rm h}$, within MC noise of the
cc-pVDZ FCI value $-15.836\,E_{\rm h}$. The recovered CI vector
$\widehat{c}$ was projected into a CAS(4,(2,2)) active space (the
textbook BeH$_2$ valence active space; CASSCF reference computed at
the same active space), the active-space Hamiltonian diagonalised in
the NN-derived natural-orbital basis, and NEVPT2 evaluated on the
resulting variational eigenvector. The result is
\begin{center}\renewcommand{\arraystretch}{1.05}\small
\begin{tabular}{lccc}
\toprule
& reference $E$ (Ha) & $E_{\rm NEVPT2}$ (Ha) & gap to FCI (mE_h) \\
\midrule
$\widehat{c}$ in NN-NOs (this work)        & $-15.7790$ & $-15.8211$ & $+15.3$ \\
CASSCF orbitals (baseline) & $-15.7744$ & $-15.8171$ & $+19.3$ \\
\midrule
FCI(cc-pVDZ) (reference)   &            & $-15.8364$ &        \\
\bottomrule
\end{tabular}
\end{center}
The $\widehat{c}$-derived CASCI reference lies $4.7\,{\rm m}E_{\rm h}$
below CASSCF at the same (4,4) active space, and the resulting
NEVPT2 closes the gap to FCI by $4.0\,{\rm m}E_{\rm h}$. The NN
orbitals import additional correlation into the active space that
CASSCF orbital optimisation cannot reach without enlarging the
active space. This is the expected qualitative behaviour: a
near-FCI-quality real-space wavefunction's natural orbitals naturally
include some of the dynamic correlation that variational CASSCF
within a small active space cannot.
...
compared against PySCF FCI in the same natural-orbital basis. Both
runs use the standard pipeline: \texttt{candidate\_tol=$10^{-4}$},
naive L${}_2$ normalisation of the sample mean. ``--'' indicates
H$_2$O cc-pVDZ was excluded because the full-FCI Hamiltonian exceeds
our 32~GB workstation memory; a CASCI(8,(4,4)) frozen-core variant
gives qualitatively similar agreement (data not shown).}
...
& $\widehat{c}$ & FCI & $\widehat{c}$ & FCI \\
\midrule
Tr $\gamma$ ($N_e$)            & $2.000000$ & $2.000000$ & $6.000000$ & $6.000000$ \\
NO occ. $n_1$ (largest)        & $1.9118$ & $1.8827$ & $1.9701$ & $2.0000$ \\
NO occ. $n_2$                  & $0.0825$ & $0.1107$ & $1.7745$ & $1.9571$ \\
$C_0$ (HF amplitude)           & $+0.978$ & $+0.970$ & $+0.902$ & $+0.978$ \\
$C_0^2$                        & $0.956$  & $0.941$  & $0.814$  & $0.957$ \\
$N_{\rm unp}$ (Head--Gordon)   & $0.170$  & $0.222$  & $0.923$  & $0.171$ \\
$|\mu|$ (Debye)                & $0$      & $0$      & $0$      & $0$     \\
$Q_{zz}$ (a.u.)                & $-1.145$ & $-1.175$ & $-8.04$  & $-8.72$ \\
\bottomrule
\end{tabular}
\end{table>
