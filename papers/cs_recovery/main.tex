...
substantially beyond the chosen Gaussian basis. The remaining
clean routes are therefore (b) a constrained-optimisation
approach that projects the gradient onto the orthogonality
manifold rather than minimising a soft penalty (sidesteps the
hyperparameter altogether), and (c) an architectural change so
that $\Psi_{\rm NN}$ lies in (or is biased toward) the chosen
Gaussian basis (e.g.\ by a Gaussian-product backbone, or by
training the NN against a basis-restricted reference). The
\texttt{OmegaQMC.vmcopt\_nn\_nes} module exposes four skeleton
implementations across the three penalty axes; the four
trajectories together delineate the design space's boundary.
