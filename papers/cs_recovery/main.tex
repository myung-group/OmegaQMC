...
FCI}\|_2 \in [0.115, 0.186]$ across multireference and
...
the recovery error ($0.16$) projects significantly onto the
...
The full \ce{H8}/cc-pVDZ row required omitting NEVPT2: PySCF's
\texttt{compare\_nevpt2} (which constructs a CASSCF baseline)
segfaults at $n_{\rm orb}=40$, CAS$(8,(4,4))$ on the GH200 build we
used; the post-HF subset (CCSD/CCSD(T)/MC-PDFT, all driven by the
recovered $\widehat c$ and its NN-NO frame) ran to completion and
the recovery error matched the other three systems
($\|c_{\widehat c}-c_{\rm FCI}\|_2 = 0.160$).
