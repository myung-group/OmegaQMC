...
On the same \ce{H2}/cc-pVDZ cell, OMP starting from the HF seed
$(0,)(0,)$ with $m=20$ random-rotation measurements and relative
tolerance $10^{-3}$ produces an 18-determinant support; the residual
falls by five orders of magnitude (from $9.6\!\times\!10^{-3}$ to
$1.9\!\times\!10^{-5}$) over 18 OMP iterations. The vector overlap
with the FCI reference is $0.993$, comparable to the Lasso-CS
($0.995$) and the identity baseline ($0.991$). The methodological
distinction is in the support: \emph{the first ten determinants
selected by OMP, in order, are exactly the ten dominant FCI
configurations} (HF + nine $\sigma_u^2$-class double excitations);
further iterations pick up spurious cross-pairs past the noise
floor. With a tighter relative tolerance ($10^{-2}$) OMP stops at
the ten-determinant true support exactly. This is genuinely
candidate-set-free deployment: starting from one HF seed, the OMP
decoder discovers the determinant support inherent to $\Psi_{\rm
NN}$ without any pre-enumeration. Reproducibility:
\texttt{OmegaQMC.cs.compressed.omp\_decode}.
