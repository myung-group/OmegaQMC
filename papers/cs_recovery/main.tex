...
\ce{BeH2}/cc-pVDZ and stretched \ce{N2}/STO-3G we drive NEVPT2 from a
recovered $\widehat c$ reference and obtain energies $0.4$--$9$~m$E_{\rm
h}$ below CASSCF+NEVPT2 at matched active spaces, with no orbital
...
$\to$ chemist's-language translation. The framework lowers the cost of
...
\subsection{Self-consistent NN-NO frame (CASCI-free deployment)}
...
determinant set. That dependency can be removed by deriving the
natural orbitals from the NN-VMC 1-RDM itself and supplying the
candidate set as a vocabulary-choice combinatorial object. Three
candidate-set strategies are admissible (CAS index structure on the
top-occupation NN-NOs; HF-rank cutoff with all $S{+}D$ excitations;
...
\paragraph{Eliminating the HF step.} The Pass-1 frame above used HF
orbitals only as one convenient orthonormal basis; HF itself plays
no physical role in the bootstrap. Replacing the RHF call with
L\"owdin orthogonalisation of the raw AOs --- $\eta_{\rm L\ddot{o}wdin} =
S^{-1/2} \chi$ where $S$ is the AO overlap matrix --- removes the
SCF iteration entirely. The empirical test on the same \ce{H2} cell
confirms unitary covariance: Pass-1 $\widehat c$ in L\"owdin basis is
spread across many AOs (the $((0),(0))$ amplitude is only $0.137$
because the L\"owdin $(0,0)$ determinant has both electrons on the
first H atom rather than one per atom), but the eigenvalues of the
resulting 1-RDM and the Pass-2 $\widehat c$ in the NN-NO frame are
\emph{identical to the HF-bootstrap result to printed precision}:
NN-NO occupations $\{1.8914, 0.0905, 0.0057, 0.0045\}$, Pass-2 HF
amplitude $+0.97035$, NN-NO/FCI-NO orbital alignment
$0.9997$ / $0.9950$ on the dominant pair (cf.\
Tab.~\ref{tab:h2_nn_no} and
\texttt{papers/cs\_recovery/data/h2\_nn\_no\_demo\_lowdin.json}).
The framework is therefore literally free of HF, CASSCF, CASCI, and
FCI on systems where the candidate set spans the full $(n_\alpha,
n_\beta)$ enumeration, as for \ce{H2}/cc-pVDZ.

\paragraph{Larger systems: candidate-set vs Pass-1-basis compatibility.}
On systems where the candidate set must be filtered (e.g.\ a fixed
amplitude threshold $|c_I| > \eta$ or a CAS truncation), the
choice of Pass-1 basis matters: the index tuples
$(\mathrm{occ}^\alpha, \mathrm{occ}^\beta)$ that the candidate set
records refer to "the $I$-th orbital of the current basis," so a
candidate set defined by amplitude in basis $A$ does not pick out
the dominant determinants in basis $B$. We verified this on a
fully-converged \ce{BeH2}/cc-pVDZ NN trial (FermiNet+J, 5000 SR
iterations at 1024 walkers, $E_{\rm NN} = -15.9021\,E_{\rm h}$;
\textbf{65.7\,m$E_{\rm h}$ below FCI(cc-pVDZ)}; see
\S\ref{sec:beh2_convergence}). With the filtered candidate set
($|c^{\rm FCI}_I| > 10^{-4}$, 2076 determinants),
HF Pass-1 produces a clean bootstrap (NN-NO occupations
$\{1.997, 1.876, 1.873, 0.030, 0.023, 0.020\}$, Pass-2 reference
amplitude $+0.952$ vs FCI's $+0.978$, orbital alignment $0.9998$ on
$1\sigma_g$), whereas L\"owdin Pass-1 fails (NN-NO occupations
$\{1.97, 0.78, 0.75, 0.60, 0.57, 0.22\}$, Pass-2 reference
amplitude $+0.142$) --- the L\"owdin basis's atom-localised
"reference" determinant is physically meaningless for \ce{BeH2}'s
delocalised valence, so the candidate set does not span the
trial's dominant content in that basis. Empirically: \emph{use
L\"owdin Pass-1 only with full-enumeration candidate sets; use HF
Pass-1 for filtered candidate sets that derive from a CI calculation
or from CAS-vocabulary indices in HF orbital ordering.} HF in this
deployment costs one SCF cycle ($\sim$~seconds for any first- or
second-row system) and does not introduce CASSCF or CI machinery.
...
(\textbf{II}) On \ce{BeH2}/cc-pVDZ and stretched \ce{N2}/STO-3G, NEVPT2
driven from a recovered $\widehat c$ reference reaches energies
$0.4$--$9$\,m$E_{\rm h}$ below CASSCF+NEVPT2 at matched active spaces,
with no orbital optimisation step --- the recipe directly attacks the
CASSCF bottleneck that has limited multi-reference perturbation theory
on transition-metal and strongly-correlated systems for decades.
...
trial evaluation, and orbital values. By delivering a chemist-readable
