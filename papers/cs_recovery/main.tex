...
converged \ce{BeH2}/cc-pVDZ, NEVPT2 driven from a recovered
$\widehat c$ reference obtains energies $0.4$--$4.0$\,m$E_{\rm h}$
\emph{below} CASSCF+NEVPT2 at matched CAS sizes
$(4{,}(2{,}2)), (8{,}(3{,}3)), (10{,}(3{,}3))$, with no orbital
...
trials (e.g.\ the BeH$_2$ cell of \S\ref{sec:beh2_convergence})
...
\subsection{Beyond identity design: random-rotation compressed sensing}
\label{sec:cs_random_rotation}

The sample-mean + Lasso scheme above has an identity design matrix
(one estimator per coefficient) and is therefore the simplest case of
the CS family. A genuine compressed-sensing variant replaces the
$N_{\rm det}$ per-coefficient estimators with $m \ll N_{\rm det}$
``compressed'' measurements drawn from random orbital rotations.

For $m$ random orthogonal matrices $\{U_j\}_{j=1}^m$ (sampled from
the Haar measure on $O(n_{\rm orb})$), the rotated HF determinant
$D_{\rm HF}^{(U_j)}(R) = \det[\eta_p^{(U_j)}(r_i)]$ is a single
Slater determinant in the rotated basis
$\eta_p^{(U_j)} = \sum_q (U_j)_{qp}\eta_q$, and its overlap with each
candidate determinant is
\begin{equation}
A_{j,I} \;=\; \langle D_{\rm HF}^{(U_j)} | D_I \rangle
       \;=\; \det\bigl(U_j[{\rm occ}_I^\alpha,:n_\alpha]\bigr)\,
            \det\bigl(U_j[{\rm occ}_I^\beta,:n_\beta]\bigr).
\end{equation}
The walker-bank measurement is the sample mean
$y_j = \langle D_{\rm HF}^{(U_j)}/\Psi_{\rm NN}\rangle = (Ac)_j/\sqrt{Z}$,
and recovery follows the canonical CS prescription
\begin{equation}
\widehat c \;=\; \arg\min_{c}\;
   \tfrac{1}{2}\bigl\|y - Ac\bigr\|_2^2 \;+\; \lambda\|c\|_1.
\end{equation}
This is no longer reducible to coordinate-wise soft-thresholding ---
$A$ is dense with rows drawn from a random orthogonal projection ---
and requires a proper Lasso solver (FISTA / coordinate descent).
Walker--determinant evaluations drop from $\mathcal{O}(K_s\,N_{\rm
det})$ for the identity-design scheme to $\mathcal{O}(K_s\,m)$, and
the sample complexity becomes the Candès--Tao
$m \gtrsim C\,K_{\rm eff}\log(N_{\rm det}/K_{\rm eff})$.

\paragraph{Empirical regime dependence.}
On \ce{H2}/cc-pVDZ ($N_{\rm det}=100$, $K_s = 12{,}800$) the random-
rotation CS variant achieves $\|\widehat c - c^{\rm FCI}\|_2 = 0.091$
using only $m=20$ measurements (20\% of $N_{\rm det}$), compared
with the identity-design baseline of $0.135$ at full $N_{\rm det}$
measurements --- a $32\%$ recovery-error reduction with a $5\times$
measurement budget reduction. On the converged \ce{BeH2}/cc-pVDZ
cell ($N_{\rm det}=2158$, $K_s = 51{,}200$), however, the random-
rotation scheme does not outperform the identity-design baseline at
any $(m, \lambda)$ tested in our sweep: the best CS recovery error
was $\sim 0.33$ at $m=200$ vs the baseline $0.30$. The regime
distinction is the walkers-per-coefficient ratio: when $K_s \gg
N_{\rm det}$ (BeH$_2$, $K_s/N_{\rm det} \approx 24$), each per-
coefficient sample mean is already low-noise and the identity design
suffices; when $K_s \lesssim N_{\rm det}$ (a system with millions of
candidate determinants but only thousands of walkers, such as
\ce{H2O}/aug-cc-pVDZ at CAS(16) where $N_{\rm det} \approx 3.3\times
10^6$), the random-rotation scheme's $\mathcal{O}(m \log N_{\rm
det})$ scaling becomes the only tractable route to CI-vector
extraction at all. The framework supports both deployments; the
choice is regime-dependent. Reproducibility:
\texttt{OmegaQMC.cs.compressed} module and
\texttt{examples/run\_cs\_compressed\_demo.py}.

\paragraph{Candidate-set-free recovery via orthogonal matching pursuit.}
The Lasso step above still requires a pre-specified candidate set
$\mathcal{S}$ for the design matrix $A$. A natural CS-family
extension --- orthogonal matching pursuit (OMP) --- replaces the
pre-specified $\mathcal{S}$ with an adaptive search starting from a
single HF reference: at each iteration the determinant whose
design-column $A_{,I}$ has highest absolute correlation with the
current residual $r = y - A_S \widehat c_S$ is added to the support,
the coefficients are re-fit by least squares, and the residual is
updated until $\|r\|$ falls below a relative tolerance.

OMP succeeds on \ce{H2}/cc-pVDZ: starting from a single HF seed
with $m=80$ random-rotation measurements and relative tolerance
$5\!\times\!10^{-2}$, the recovered 12-determinant support contains
the ten dominant FCI configurations (HF + nine $\sigma_u^2$-class
doubles) and gives vector overlap $0.996$ with FCI, comparable to
Lasso-CS ($0.997$). The discovered support is found from the seed,
without pre-enumeration.

\paragraph{OMP scaling limit on larger active spaces.}
On the converged \ce{BeH2}/cc-pVDZ ($N_{\rm det}=2092$,
$K_{\rm eff} \approx 45$) and \ce{H4}/cc-pVDZ ($N_{\rm det}=1569$,
$K_{\rm eff} \approx 51$) cells, OMP fails to recover the FCI
support and overshoots into noise-correlated determinants
(typical recovered overlap with FCI $\sim 0.2$--$0.6$). The
underlying problem is column-to-noise SNR: a single column of $A$
has entries
$A_{j,I} = \det U_j[{\rm occ}_I^\alpha,:n_\alpha]\,
\det U_j[{\rm occ}_I^\beta,:n_\beta]$
that scale as $\mathcal{O}((1/\sqrt{n_{\rm orb}})^{N})$ --- of
order $10^{-3}$ for the dominant FCI determinant in \ce{BeH2}
($n_{\rm orb}=24$, $N=6$). The residual noise from finite walker
sampling is of order $1/\sqrt{K_s} \approx 4\!\times\!10^{-3}$, so
the per-column signal sits at or below the noise floor, and OMP
greedily picks noise-aligned columns instead of the true support.
Lasso-CS sidesteps this because the L1 step uses the joint
information across all columns simultaneously; OMP's
column-by-column greedy requires per-column SNR $>1$ to make
correct selections.

The practical take-away: OMP is a viable candidate-set-free decoder
only when per-column SNR is comfortable ($\sim 1$ or better) ---
typically small systems ($N \le 4$ electrons) or large walker banks
($K_s \gtrsim 10^5$ on first-row atoms). For larger active spaces
the Lasso-CS variant with a pre-specified pool is the workable
production decoder; the candidate-pool definition is the cost paid
to keep the support discovery noise-robust.

\paragraph{Hybrid identity-warm-start OMP does not rescue scaling.}
A natural attempt to overcome the column-SNR limit is to seed OMP
with the support from the identity-design recovery: run
\S\ref{sec:theory}'s identity-design CS first, identify the top-$K$
candidates by $|\widehat c_I^{\rm id}|$, then iterate OMP on the
random-rotation measurements starting from that support. We
implemented this as
\texttt{OmegaQMC.cs.compressed.hybrid\_omp\_decode}. On the same
\ce{BeH2}/cc-pVDZ cell the hybrid scheme is no better than pure
OMP: at $K=10$--$100$ seed candidates and $m=100$--$300$
measurements, the recovered overlap remains $\le 0.36$ versus the
identity-design baseline of $0.96$. The OMP iterations
\emph{overwrite} the good seed coefficients via least-squares refit
on the low-SNR random-rotation measurements, propagating noise into
the refined vector. The least-squares step is where the SNR limit
binds; OMP-specific support discovery cannot bypass it.

The proper fix is structured measurements: replacing the random
orthogonal rotations $U_j$ with Givens-style occupied$\to$virtual
mixings that preferentially probe specific excitation classes, so
that per-column design-matrix entries scale as $\mathcal{O}(1)$ on
the targeted class rather than as
$\mathcal{O}\bigl((1/\sqrt{n_{\rm orb}})^N\bigr)$ uniformly. We
treat this as an interesting follow-up direction; the present
work establishes that for $N_{\rm det} \lesssim 10^3$ the
random-rotation Lasso variant works as canonical CS, while for
larger active spaces the identity-design pipeline of
\S\ref{sec:theory} remains the working production decoder.

Reproducibility data:
\texttt{papers/cs\_recovery/data/omp\_decoder\_sweep.json};
sweep script \texttt{examples/run\_cs\_decoder\_sweep.py}.
...
\State $\widehat c^{\rm NN-NO,\,\rm Lasso}_I \gets
       ...
       \Comment{soft-thresholding (\S\ref{sec:cs_connection});
...
\S\ref{sec:beh2_convergence}) the recovered
...
captured by the FermiNet+J ansatz (\S\ref{sec:beh2_convergence}).
The basis-projected $\widehat c$ recovered via the HF-Pass-1,
NN-NO-Pass-2 bootstrap (\S\ref{sec:nn_no_self_consistent}) carries
...
\S\ref{sec:nn_no_self_consistent} where the basis-projected fraction
...
\S\ref{sec:nn_no_self_consistent} (the trial cannot be projected
...
\subsection{Selected-CI warm-start with $\widehat c$}
