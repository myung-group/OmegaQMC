...
Tested on the H$_2$/cc-pVDZ pilot at $R = 2.5\,a_0$ via the SLURM
demo in \texttt{scripts/slurm\_cs/run\_h2\_pfau\_spectroscopy.sh}
running on one Grace--Hopper (GH200) GPU, the full pipeline
produces in a single $\sim 30$~minute job: the trained Pfau-NES
$(\psi_1, \psi_2)$, the recovered $\widehat{c}^{(0)},
\widehat{c}^{(1)}$, the transition dipole and oscillator strength,
the dominant NTO pair, and the per-state NEVPT2 totals --- all
serialised into a JSON summary suitable for direct Table insertion.
The 13 unit tests in
\texttt{tests/test\_cs\_transition.py} and
\texttt{tests/test\_cs\_mrpt\_multistate.py} validate each routine
against PySCF's own \texttt{trans\_rdm1s} and \texttt{NEVPT}
