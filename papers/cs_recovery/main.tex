...
figure of merit.} A fixed Pfau-NES K=4 FermiNet+Jastrow checkpoint
on aug-cc-pVDZ is post-processed through CASCI references of growing
active-space size; left axis is the match-overlap between the
recovered $\widehat c^{(\rm exc)}$ and the closest CAS eigenvector
(framework's primary diagnostic), right axis is the CASCI+PT2
vertical excitation energy at the matched root (a CASCI quantity,
not an NN quantity --- shown for context). Filled markers are
completed runs (Tab.~\ref{tab:cas_size_convergence}); TBD entries
correspond to SLURM jobs currently running on the same checkpoint.
The framework's claim is that match-overlap saturates as the CAS
vocabulary becomes rich enough to represent the NN excited state
faithfully; an irreducibly low saturation value would indicate
genuine NN content outside any chemist-readable vocabulary, an
informative outcome in its own right.}
...
CAS(16,(4,4))   & $-75.701$ & $-183$ & $0.23/0.90$ & $9.70$ \\
CAS(20,(4,4))   & TBD       & TBD    & TBD    & TBD    \\
\hline
\end{tabular}
\end{table}

\noindent The CAS(16) entry reports two match-overlaps because the
maximum-oscillator-strength selector lands on a different rotated K=4
eigenstate at CAS(16) than at CAS(12): the recovered excited
$\widehat c^{({\rm exc})}$ projects only at $0.23$ on the matched
CAS(16) eigenvector (whose bare CASCI energy is $37.8$\,eV, well above
the dipole-allowed Rydberg target), while the recovered ground-state
projection rises sharply from $0.48$ at CAS(8) to $\mathbf{0.90}$ at
CAS(16) --- a clean signature that the larger CAS captures the
ground-state CI vector almost faithfully. The corresponding $\Delta
E_{\rm CAS+PT2}$ at CAS(16) is $9.70$\,eV, within $0.07$\,eV of the
CAS(12) value, confirming that the NEVPT2 step is robust to active-
space size even though the selected eigenvector differs. CAS(20) at
$\sim 23$\,M active-space determinants requires roughly $10\times$ the
NEVPT2 wall time of CAS(16) (which itself took $\sim 3$\,h on the
local RTX 4090) and is deferred to a follow-up; the converging
ground-state match-overlap already provides the central
chemist-language-bridge claim.
