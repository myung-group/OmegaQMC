...
import jax  # JAX for automatic differentiation and JIT compilation
import jax.numpy as jnp  # JAX's NumPy-like interface
...
class ShellType:
    ...
    Used to store information about atomic orbitals in quantum chemistry calculations.
    """
...
def read_shell(ish_basis, ia, nsgs, ncgs):
    ...
    if am == 0:
        # For s-orbitals (l=0), there is one shell in both cartesian and spherical coordinates
        ...
        # For p-orbitals (l=1), there are 3 components (px, py, pz) in both cartesian and spherical
        ...
        # - 5 spherical components (d_{z^2}, d_{xz}, d_{yz}, d_{x^2-y^2}, d_{xy})
...
def read_shell_two(ish_basis, ia, nsgs, ncgs):
    """
    Reads and processes a split-valence basis shell from PySCF basis set format.
    ...
    Returns:
        shell1, shell2: Two ShellType objects containing processed basis shell information
    """
    # Extract angular momentum from first element
    am = ish_basis[0]
    # Count number of primitive Gaussians
    ...
    if am == 0:
        # For s-orbitals (l=0), each shell has one component in both cartesian and spherical
        ...
        # Lists to store exponents and normalized coefficients for both contractions
        ...
        # For p-orbitals (l=1), each shell has 3 components (px, py, pz) in both cartesian and spherical
...
def get_psi_fun(mf):
    """
    Creates functions for evaluating the wavefunction and local energy components
    from a PySCF mean-field calculation.
    
    Args:
        mf: PySCF mean-field object containing molecular and electronic structure information
    ...
    def H_psi_over_psi(elec_crds, nuc_crds, params_vmc):
        ...
        for i in range(elec_crds.shape[0]):
            ...
            hess_func_i = jax.hessian(lambda r_i_var: 
                                    log_psi_of_single_electron_crds(r_i_var, i))
            lap_log_psi_sum += jnp.trace(hess_func_i(elec_crds[i]))

            # Calculate gradient squared for electron i
            grad_func_i = jax.grad(lambda r_i_var:
                                 log_psi_of_single_electron_crds(r_i_var, i))
    ...
    def local_energy_en(elec_crds, nuc_crds):
        ...
        en_pot_energy = classical_inter_coulomb_energy(elec_crds, nuc_crds, 
                                                     e_charges, Z_charges)
    ...
    def local_energy_ke(elec_crds, nuc_crds, params_vmc):
        ...
        grad_log_psi = jax.grad(log_trial_wavefunction, argnums=0) (elec_crds,
                                                                    nuc_crds,
                                                                    params_vmc)
