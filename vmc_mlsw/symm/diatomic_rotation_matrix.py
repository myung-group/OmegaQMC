...
def demonstrate_diatomic_rotation():
    ...
    print(f"Error: {jnp.linalg.norm(rotated1 - jnp.array([0, 0, jnp.linalg.norm(vector1)])):.2e}\n")
    
    # Example 2: Diatomic molecule coordinates
    print("Example 2: H2 molecule")
    A_H2 = jnp.array([0.0, 0.0, 0.0])  # First H atom
    B_H2 = jnp.array([0.74, 0.0, 0.0])  # Second H atom (H-H bond length ~0.74 Å)
    AB_vector = B_H2 - A_H2
    ...
    print(f"Bond length preserved: {jnp.linalg.norm(AB_rotated):.6f} (original: {jnp.linalg.norm(AB_vector):.6f})\n")
    ...
    print(f"Z-alignment check: {jnp.abs(AB_CO_rot[0]):.2e}, {jnp.abs(AB_CO_rot[1]):.2e}, {AB_CO_rot[2]:.6f}\n")
    
    # Verification: Check that rotation matrices are orthogonal
    print("=== Matrix Properties Verification ===")
    for i, (name, R) in enumerate([("Example 1", R1), ("Example 2", R2), ("Example 3", R3)], 1):
        det_R = jnp.linalg.det(R)
        orthogonality_error = jnp.linalg.norm(R @ R.T - jnp.eye(3))
        print(f"{name}: det(R) = {det_R:.6f}, orthogonality error = {orthogonality_error:.2e}")
