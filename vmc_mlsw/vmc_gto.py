...
def get_vmc_func (mf, 
    ...
    def vmc_energy():
        ...
        ener_en_samples = jax.vmap(local_energy_en, in_axes=(0, None))(stacked_samples, nuc_crds)
    ...
    def vmc_gradient_prep():
        ...
        print(f"Processing {num_samples} samples with {num_electrons} electrons and {num_nuc} nuclei")
        ...
        def compute_gradient_batch(samples_batch):
            ...
            grad_logpsi_elc, grad_logpsi_nuc = jax.vmap(grad_fn_logpsi)(samples_batch)
    
            return grad_ee, grad_en_elc, grad_en_nuc, grad_ke_elc, grad_ke_nuc, grad_logpsi_elc, grad_logpsi_nuc
        ...
        for key, elec_samples in dict_elec_samples.items():
            ...
            for batch_idx in range(num_batches):
                ...
                # Compute all gradients for this batch using JIT-compiled function
                ...
                # Store results
                grad_eloc_ee_elc = grad_eloc_ee_elc.at[start_idx:end_idx].set(grad_ee_batch)
                grad_eloc_en_elc = grad_eloc_en_elc.at[start_idx:end_idx].set(grad_en_elc_batch)
                grad_eloc_en_nuc_base = grad_eloc_en_nuc_base.at[start_idx:end_idx].set(grad_en_nuc_batch)
                grad_eloc_ke_elc = grad_eloc_ke_elc.at[start_idx:end_idx].set(grad_ke_elc_batch)
                grad_eloc_ke_nuc_base = grad_eloc_ke_nuc_base.at[start_idx:end_idx].set(grad_ke_nuc_batch)
                grad_logpsi_elc = grad_logpsi_elc.at[start_idx:end_idx].set(grad_logpsi_elc_batch)
                grad_logpsi_nuc_base = grad_logpsi_nuc_base.at[start_idx:end_idx].set(grad_logpsi_nuc_batch)
